...
export const TransportLogJefePage = () => {
  ...
  const uniqueEmpresas = useMemo(() => {
    ...
    Object.values(VEHICLE_COMPANY_LABELS).forEach((label) => empresas.add(label));

    transportLogs.forEach((log) => {
      const internalCompany = getInternalCompanyFromLog(log);
      if (internalCompany) empresas.add(VEHICLE_COMPANY_LABELS[internalCompany]);
    });
  ...
  const handleDownloadExcel = () => {
    ...
    const exportData = filteredLogs.map((log) => {
      ...
      return {
        ...
        'Tiempo Viaje': getDuration(log.departureAt || log.createdAt, log.arrivalAt),
  ...
  const handleSaveM3 = async () => {
    ...
    try {
      ...
      const updated = await transportLogJefeService.correctMaterial(detailLog.id, {
  ...
  const filteredLogs = useMemo(() => {
    return transportLogs.filter((log) => {
      ...
      const matchesProveedor = !proveedorFilter || (getEmpresaType(log) === 'EXTERNO' && log.owner?.companyname === proveedorFilter);
      const type = getEmpresaType(log);
      const matchesEmpresa = !empresaFilter || type === empresaFilter;
      const matchesEmpresaNombre = !empresaNombreFilter || (type === 'INTERNO' && getEmpresaLabel(log) === empresaNombreFilter);
      const matchesCliente = !clienteFilter || String(log.clientId ?? log.client?.id ?? '') === clienteFilter;
      ...
      const matchesPlaca = !placaFilter || log.vehicle?.plate?.toLowerCase().includes(placaFilter.toLowerCase());
      ...
      return matchesSearch && matchesStatus && matchesProveedor && matchesEmpresa && matchesEmpresaNombre && matchesCliente && matchesObra && matchesMaterial && matchesDate && matchesPlaca;
    });
  }, [transportLogs, searchTerm, statusFilter, proveedorFilter, empresaFilter, empresaNombreFilter, clienteFilter, obraFilter, materialFilter, dateFrom, dateTo, placaFilter]);

  useEffect(() => {
    setPage(1);
  }, [searchTerm, statusFilter, proveedorFilter, empresaFilter, empresaNombreFilter, clienteFilter, obraFilter, materialFilter, dateFrom, dateTo, placaFilter]);
  ...
  const disableValidateBtn = detailStatus === 'REVISADO' || detailStatus === 'VALIDADO';
  const disableEditBtn = detailStatus === 'REVISADO' || detailStatus === 'VALIDADO';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Validación del Material</h1>
        ...
        {!detailLoading && detailLog && (
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg text-sm">
              ...
              <p className={`font-semibold text-lg ${resolvedM3.difference === null
                ? 'text-gray-400'
