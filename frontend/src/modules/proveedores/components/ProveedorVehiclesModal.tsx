...
export const ProveedorVehiclesModal = ({
  proveedorId,
}: ProveedorVehiclesModalProps) => {
    ...
    const fetchVehicles = async () => {
      try {
        ...
        const filtered = vehicles.filter(
          (vehicle) => vehicle.ownerId === Number(proveedorId),
        );
  ...
  const columns = [
    ...
    { header: "ID Carro", accessor: "vehicleid" as keyof Vehicle },
    {
      header: "Estado",
    ...
    <div className="mt-4">
      {vehicles.length === 0 && !isLoading ? (
        <p className="text-gray-500 text-center py-4">
          No hay vehículos registrados para este proveedor.
        </p>
      ) : (
